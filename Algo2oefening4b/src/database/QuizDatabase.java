...
public class QuizDatabase {
	...
	public ArrayList<Quiz> getQuizzenlijst(){
		return this.quizzen;
	}
	
	public Quiz getQuiz(String onderwerp, String klascode, String auteur){
		boolean stop = false;
		int i = 0;
		for(i=0; i< quizzen.size() && stop==false;i++){
			if(quizzen.get(i).getOnderwerp().equals(onderwerp) &&
					quizzen.get(i).getKlascode().equals(klascode) &&
					quizzen.get(i).getAuteur().equals(auteur)){
				stop = true;
			}
		}
		return quizzen.get(i-1);
	}
